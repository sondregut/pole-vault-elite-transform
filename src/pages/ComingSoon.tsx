...
const ComingSoon = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      // Store the free download submission
      const { error: dbError } = await supabase
        .from('free_download_submissions')
        .insert({
          product_id: 13, // Best Pole Vault Drills product ID
          product_name: "Best Pole Vault Drills",
      ...
      if (dbError) {
        ...
        // Don't block the download for database errors
      }

      // Use direct public URL for download
