...
exports.getPoleAnalysis = getPoleAnalysis;
exports.getHeightProgression = getHeightProgression;
exports.analyzeTechnique = analyzeTechnique;
...
async function searchJumps(userId, params) {
    ...
    snapshot.docs.forEach(doc => {
        ...
        if (params.location && !((_a = session.location) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(params.location.toLowerCase())))
            return;
        if (params.sessionType && session.sessionType !== params.sessionType)
            return;
...
async function comparePerformance(userId, params) {
    const db = admin.firestore();
    const sessionsRef = db.collection('users').doc(userId).collection('sessions');
    ...
    const filterSessions = (criteria) => {
        return sessions.filter(s => {
            var _a, _b, _c;
            const sessionDate = ((_a = s.date) === null || _a === void 0 ? void 0 : _a.toDate) ? s.date.toDate() : new Date(s.date);
            if (criteria.dateFrom) {
                if (sessionDate < new Date(criteria.dateFrom))
                    return false;
            }
            if (criteria.dateTo) {
            ...
            if (criteria.location) {
                if (!((_b = s.location) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(criteria.location.toLowerCase())))
                    return false;
            }
            if (criteria.sessionType) {
                if (((_c = s.sessionType) === null || _c === void 0 ? void 0 : _c.toLowerCase()) !== criteria.sessionType.toLowerCase())
                    return false;
            }
            if (criteria.isIndoor !== undefined) {
                if (s.isIndoor !== criteria.isIndoor)
                    return false;
            }
            return true;
        });
...
function parsePoleLength(length) {
    if (!length)
        return 0;
...
async function getPoleAnalysis(userId, params) {
    const db = admin.firestore();
    const sessionsRef = db.collection('users').doc(userId).collection('sessions');
    ...
    snapshot.docs.forEach(doc => {
        ...
        jumps.forEach((jump) => {
            ...
            // Apply filters
            if (params.poleBrand && !((_a = poleDetails.brand) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(params.poleBrand.toLowerCase())))
                return;
            if (params.poleLength && !((_b = poleDetails.length) === null || _b === void 0 ? void 0 : _b.includes(params.poleLength)))
                return;
    ...
    if (poles.length > 0) {
        ...
        const sortedBySize = [...poles].sort((a, b) => {
            if (b.lengthInches !== a.lengthInches)
                return b.lengthInches - a.lengthInches;
    ...
    if (params.targetHeight) {
        const polesAtHeight = poles.filter(p => p.heights.some(h => Math.abs(h - params.targetHeight) < 0.15));
    ...
    return {
        ...
        recommendedForHeight: recommendedPole ? {
            ...
            attemptsAtHeight: recommendedPole.heights.filter(h => Math.abs(h - (params.targetHeight || 0)) < 0.15).length
        } : null,
        bestOverall: poles[0] ? {
            pole: poles[0].name,
...
async function getHeightProgression(userId, params) {
    const db = admin.firestore();
    const sessionsRef = db.collection('users').doc(userId).collection('sessions');
    ...
    snapshot.docs.forEach(doc => {
        var _a;
        const session = doc.data();
        const sessionDate = ((_a = session.date) === null || _a === void 0 ? void 0 : _a.toDate) ? session.date.toDate().toISOString() : session.date;
        const jumps = session.jumps || [];
        jumps.forEach((jump) => {
            const height = parseHeight(jump.height);
    ...
    if (attempts.length >= 10) {
        ...
        if (last5Rate > first5Rate + 0.1)
            trend = 'improving';
        else if (last5Rate < first5Rate - 0.1)
            trend = 'declining';
    }
    // Readiness assessment
    let readiness = 'not_ready';
    if (successRate >= 66)
        readiness = 'ready_to_move_up';
    else if (successRate >= 50)
        readiness = 'close';
    else if (successRate >= 33)
        readiness = 'developing';
    // Cast to resolve TypeScript narrowing issue with closures
    const firstMakeResult = firstMake;
    const lastMakeResult = lastMake;
    return {
...
async function analyzeTechnique(userId, params) {
    const db = admin.firestore();
    const sessionsRef = db.collection('users').doc(userId).collection('sessions');
    const snapshot = await sessionsRef.get();
    ...
    snapshot.docs.forEach(doc => {
        ...
        jumps.forEach((jump) => {
            const height = parseHeight(jump.height);
            if (targetHeight && Math.abs(height - targetHeight) > heightTolerance)
                return;
            const isMake = jump.result === 'make';
            // Grip height
            if (jump.gripHeight) {
                if (!gripMap[jump.gripHeight])
                    gripMap[jump.gripHeight] = { makes: 0, attempts: 0 };
                gripMap[jump.gripHeight].attempts++;
                if (isMake)
                    gripMap[jump.gripHeight].makes++;
            }
            // Steps
            if (jump.steps) {
                const stepCount = parseInt(jump.steps);
                if (!isNaN(stepCount)) {
                    if (!stepsMap[stepCount])
                        stepsMap[stepCount] = { makes: 0, attempts: 0 };
                    stepsMap[stepCount].attempts++;
                    if (isMake)
                        stepsMap[stepCount].makes++;
                }
            }
            // Takeoff
            if (jump.takeOff) {
                if (!takeoffMap[jump.takeOff])
                    takeoffMap[jump.takeOff] = { makes: 0, attempts: 0 };
                takeoffMap[jump.takeOff].attempts++;
                if (isMake)
                    takeoffMap[jump.takeOff].makes++;
            }
            // Standards
            if (jump.standards) {
                if (!standardsMap[jump.standards])
                    standardsMap[jump.standards] = { makes: 0, attempts: 0 };
                standardsMap[jump.standards].attempts++;
                if (isMake)
                    standardsMap[jump.standards].makes++;
            }
        });
    });
...
async function getTrainingRecommendations(userId, params) {
